...
execute as @s at @s if entity @e[type=minecraft:armor_stand,name="map-center"] if score lobby game-state matches 1 run scoreboard players set gambling custom 0
execute as @s at @s if entity @e[type=minecraft:armor_stand,name="map-center"] if score lobby game-state matches 1 run tellraw @s {"text":"[Gambling] has been disabled","color":"red"}
