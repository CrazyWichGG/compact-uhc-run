...
execute as @s at @s if entity @e[type=minecraft:armor_stand,name="map-center"] if score lobby game-state matches 1 run scoreboard players set gun custom 0
execute as @s at @s if entity @e[type=minecraft:armor_stand,name="map-center"] if score lobby game-state matches 1 run tellraw @s {"text":"[Gun] has been disabled","color":"red"}
